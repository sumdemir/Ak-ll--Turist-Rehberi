...
$(document).ready(function (e) {
	...
	document.getElementById('search-by-location-name').addEventListener('submit', searchRestaurant);
	document.getElementById('search-by-city-name').addEventListener('submit', searchHotels);
...
function searchRestaurant(e) {
	e.preventDefault();
	var placeLocation = this.placeLocation.value;
	...
	var url = `https://nominatim.openstreetmap.org/search?format=json&amenity=restaurant&country=tr&state=${placeLocation}`
...
function drawRestaurantMarkers(list) {
	...
	for (let i = 0; i < list.length; i++) {
		...
		var popUpContentWithSubCat = "<div class=popup-title>" + list[i].display_name + "</div><div class=popup-info>" + Math.round((list[i].lat) * 1000) / 1000 + " " + Math.round((list[i].lon) * 1000) / 1000 + "</div><div class=popup-info>" + "Restorant" + "</div><div class=popup-info>" + "Yeme İçme" + "</div>";

		var s = L.marker([list[i].lat, list[i].lon], { icon: color[10] });
	...
	refreshLayer([list[0].lat, list[0].lon]);
}

function drawHotelMarkers(list) {
	var color = ['', blueIcon, greenIcon, redIcon, purpleIcon, yellowIcon, orangeIcon, greyIcon, azureIcon, ochreIcon, pinkIcon, blackIcon];

	for (let i = 0; i < list.length; i++) {

		var popUpContentWithSubCat = "<div class=popup-title>" + list[i].display_name + "</div><div class=popup-info>" + Math.round((list[i].lat) * 1000) / 1000 + " " + Math.round((list[i].lon) * 1000) / 1000 + "</div><div class=popup-info>" + "Hotel" + "</div><div class=popup-info>" + "Konaklama" + "</div>";

		var s = L.marker([list[i].lat, list[i].lon], { icon: color[8] });
		s.bindPopup(popUpContentWithSubCat);
		s.addTo(lodging2);
		num1++;
...
function generateSelectOptions() {
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
			...
			let ele = document.getElementById('selectCity');
			for (let i = 0; i < cities.cityset.CITY.length; i++) {
				// BIND DATA TO <select> ELEMENT.
				ele.innerHTML = ele.innerHTML +
					'<option value="' + cities.cityset.CITY[i].cityid + '">' + cities.cityset.CITY[i].cityname + '</option>';
			}
...
function generateDistricts(ele) {
	let element = document.getElementById('selectDistrict');
	element.innerHTML = '';
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
			...
			let element = document.getElementById('selectDistrict');
			let selectedCityIndex = ele.selectedIndex;
			selectedCity = cities.cityset.CITY[selectedCityIndex];
			selectedDistrict = selectedCity.DISTRICT[0].DISTNAME; // selectedDistrict'i global değişken olarak güncelliyoruz.

			for (let i = 0; i < selectedCity.DISTRICT.length; i++) {
				// BIND DATA TO <select> ELEMENT.
				element.innerHTML = element.innerHTML +
					'<option value="' + selectedCity.DISTRICT[i].DISTID + '">' + selectedCity.DISTRICT[i].DISTNAME + '</option>';
			}

			selectedDistrict = selectedCity.DISTRICT[0].DISTNAME;
		}
	};
	xhr.open(method, url, true);
	xhr.send();
...
function searchHotels(e) {
	...
	var placeHotel = this.placeHotel.value;
	if (placeHotel.length <= 1) {
		printError("Please, provide at least 2 characters1");
		return;
	}

	if (!selectedDistrict) {
		printError("Please select a district first");
		return;
	}

	runProgressbar('digestLoader');

	var url = `https://nominatim.openstreetmap.org/search?format=json&amenity=hotel&country=tr&state=${selectedCity.cityname}&city=${selectedDistrict.DISTNAME}`
...
function onChangeDistrict(district) {


	let selectedDistrictIndex = district.selectedIndex;

	selectedDistrict = selectedCity.DISTRICT[selectedDistrictIndex];
...
function createTable(data) {
	...
	r = document.createElement("TR");
	var tableHeaders = ['Address', 'Category', 'Name'];


	for (var i = 0; i < tableHeaders.length; i++) {
		d = document.createElement("TH");
		d.setAttribute("class", 'results-heading');
		t = document.createTextNode(tableHeaders[i]);
		d.appendChild(t);
	...
	for (var j = 0; j < data.length; j++) {
		...
		for (var i = 0; i < tableHeaders.length; i++) {
			...
			if (tableHeaders[i] === tableHeaders[0]) {
				
				l = document.createElement("A");
				
				l.setAttribute("onclick", `setMapCenter(${data[j].lat},${data[j].lon})` ); 
				t = document.createTextNode(data[j].display_name);
				l.appendChild(t); 
				d.appendChild(l);
			...
			else if (tableHeaders[i] === tableHeaders[1]) {
				
				t = document.createTextNode("Hotel");
				d.appendChild(t);
				
			}

			else {
				t = document.createTextNode(data[j].name);
				d.appendChild(t);
